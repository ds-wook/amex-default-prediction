...
def _main(cfg: DictConfig):
    ...
    tab_oof = np.load(path / cfg.model.tab_oof)
    xgb_preds = pd.read_csv(path / cfg.output.name / cfg.output.xgb_preds)
    tab_preds = pd.read_csv(path / cfg.output.name / cfg.output.tab_preds)

    oofs = [xgb_oof.oof_preds, tab_oof.oof_preds]

    preds = [xgb_preds.prediction.to_numpy(), tab_preds.prediction.to_numpy()]
    ...
    print(amex_metric(target.to_numpy(), tab_oof.oof_preds))
